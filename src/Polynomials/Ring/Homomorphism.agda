...
mutual
  ...
  ⊞-ne-hom (ℕ.equal i) x xs y ys ρ Ρ =
    ...
    ∎
  ⊞-ne-hom (ℕ.less i k) = ⊞-ne-l-step-hom i k
  ⊞-ne-hom (ℕ.greater j k) x xs y ys ρ Ρ = ⊞-ne-l-step-hom j k y ys x xs ρ Ρ ︔ +-comm _ _
  ...
  ⊞-ne-l-step-hom : ∀ {n} i k
                  ...
                  → (ρ : Carrier)
                  → (Ρ : Vec Carrier n)
                  → (⟦ fst~ x ⟧ Ρ + ⟦ ⊞-ne-l k xs y ys ⟧ (ρ ∷ Ρ) * ρ) * ρ ^ i
  ...
  ⊞-ne-l-step-hom i k x xs y ys ρ Ρ =
    ...
    ≈⟨ ≪* +≫ ≪* ⊞-ne-l-hom k xs y ys ρ Ρ ⟩
      (x′ + (xs′ + ⟦ (y , k) ∷ ys ⟧ (ρ ∷ Ρ)) * ρ) * ρ ^ i
    ≈⟨ ≪* +≫ distribʳ ρ _ _ ⟩
      (x′ + (xs′ * ρ + ⟦ (y , k) ∷ ys ⟧ (ρ ∷ Ρ) * ρ)) * ρ ^ i
    ≈⟨ ≪* sym (+-assoc x′ _ _) ⟩
      (x′ + xs′ * ρ + ⟦ (y , k) ∷ ys ⟧ (ρ ∷ Ρ) * ρ) * ρ ^ i
    ≈⟨ distribʳ (ρ ^ i) _ _ ⟩
...
mutual
  ...
  ⊠-coeffs-hom ((x , i) ∷ xs) ((y , j) ∷ ys) ρ Ρ =
    ...
    ≡⟨⟩
      ⟦ (fst~ x ⊠ fst~ y , j ℕ.+ i) ∷↓ fst~ x ⋊ ys ⊞ ⊠-shift (fst~ y) ys xs ⟧ (ρ ∷ Ρ)
    ≈⟨ ∷↓-hom _ _ _ ρ Ρ ⟩
      (⟦ fst~ x ⊠ fst~ y ⟧ Ρ + ⟦ fst~ x ⋊ ys ⊞ ⊠-shift (fst~ y) ys xs ⟧ (ρ ∷ Ρ) * ρ) * ρ ^ (j ℕ.+ i)
    ≈⟨ ≪* ⊠-step-hom (fst~ x) xs (fst~ y) ys ρ Ρ ⟩
